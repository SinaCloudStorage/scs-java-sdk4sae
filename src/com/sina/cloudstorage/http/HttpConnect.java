...
//        HttpEntity entity = null;
        ...
        while (true) {
			...
			try {
                ...
                this.connection = httpRequestFactory.createHttpRequest(request, config, executionContext, redirectedURI);
                 
				if (requestCount > 1) { // retry
					pauseBeforeNextRetry(request.getOriginalRequest(),
							retriedException, requestCount,
							config.getRetryPolicy());
				}

                if (isRequestSuccessful(this.connection.getResponseCode())) {
                    /*
                     * If we get back any 2xx status code, then we know we should
                     * treat the service call as successful.
                     */
                    leaveHttpConnectionOpen = responseHandler.needsConnectionLeftOpen();
                    HttpResponse httpResponse = createResponse(request, this.connection);
                ...
                } else if (isTemporaryRedirect(this.connection)) {
                    /*
                     ...
                     */
                    String redirectedLocation = this.connection.getHeaderField("Location");;
...
//                    awsRequestMetrics.addProperty(Field.AWSRequestID, null);
                    ...
                    SCSServiceException ase = handleErrorResponse(request, errorResponseHandler, this.connection);
...
//                    awsRequestMetrics.addProperty(Field.StatusCode, ase.getStatusCode());
                    ...
                    if(RetryUtils.isClockSkewError(ase)) {
                        int timeOffset = parseClockSkewOffset(this.connection, ase);
...
//                throw handleUnexpectedFailure(e, awsRequestMetrics);
                ...
                if (!leaveHttpConnectionOpen) {
                    try {
                        ...
                        log.warn("Cannot close the connection content.", e);
                    }
                }
            }
        } /* end while (true) */
     ...
     */
    private HttpResponse createResponse(Request<?> request, HttpURLConnection conn) throws IOException {
        HttpResponse httpResponse = new HttpResponse(request);
//        if (conn.getHeaderField(Headers.CONTENT_LENGTH) != null 
//        		&& !conn.getHeaderField(Headers.CONTENT_LENGTH).equals("0")){
//        	httpResponse.setContent(conn.getInputStream());
//        }

        if (isRequestSuccessful(conn.getResponseCode())){
        	httpResponse.setContent(conn.getInputStream());
        }else{
        	httpResponse.setContent(conn.getErrorStream());
        }
        
        httpResponse.setStatusCode(conn.getResponseCode());
        httpResponse.setStatusText(conn.getResponseMessage());

        Map<String, List<String>> responseHeaders = conn.getHeaderFields();
		for(Entry<String, List<String>> entry : responseHeaders.entrySet())
			httpResponse.addHeader(entry.getKey(), ServiceUtils.join(entry.getValue()));
    ...
    private <T> T handleResponse(Request<?> request,
            HttpResponseHandler<SCSWebServiceResponse<T>> responseHandler,
            HttpResponse httpResponse, ExecutionContext executionContext) throws IOException
    {
...
//            throw e;
    ...
    private static boolean isTemporaryRedirect(HttpURLConnection conn) throws IOException {
    ...
    private SCSServiceException handleErrorResponse(Request<?> request,
            HttpResponseHandler<SCSServiceException> errorResponseHandler, HttpURLConnection conn
            ) throws IOException {

        int status = conn.getResponseCode();
        HttpResponse response = createResponse(request, conn);
...
//            requestLog.debug("Received error response: " + exception.toString());
            ...
            } else if (status == 503 && "Service Unavailable".equalsIgnoreCase(conn.getResponseMessage())) {
...
//        }
    ...
    private int parseClockSkewOffset(HttpURLConnection conn, SCSServiceException exception) {
        DateUtils dateUtils = new DateUtils(); 
        Date deviceDate = new Date();
        Date serverDate = null;
        String serverDateStr = null;
        String responseDateHeader = conn.getHeaderField("Date");
        try {
    ...
    /**
     * Shuts down this HTTP connection object, releasing any resources that might be
