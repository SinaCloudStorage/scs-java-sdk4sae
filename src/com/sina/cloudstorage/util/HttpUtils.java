    ...
    public static String encodeParameters(Request<?> request) {
    	...
    	if (request.getParameters().size() > 0) {
            for (Entry<String, String> entry : request.getParameters().entrySet()) {
            	final String encodedName = encode(entry.getKey(), DEFAULT_ENCODING);
                final String encodedValue = encode(entry.getValue(), DEFAULT_ENCODING);
                if (encodedParams.length() > 0) {
                	encodedParams.append(PARAMETER_SEPARATOR);
                }
                encodedParams.append(encodedName);
                if (encodedValue != null) {
                	encodedParams.append(NAME_VALUE_SEPARATOR);
